...
public class Account_Driver {
    ...
    public static void applyInterest(Account accounts [], int count, Scanner keyboard) {
        ...
        if(index >= 0) {
            ...
            if(accounts[index] instanceof Savings_Account) {
                ((Savings_Account)accounts[index]).applyInterest();
            }
        } else {
            System.out.println("No account exist with AccountNumber: " + accountNumber);
        }
    ...
    public static Account createAccount(Scanner keyboard) {
        ...
        if(choice == 1)  { // Checking account
            ...
            account = new Checking_Account(accountNumber, fee);
        } else { // Savings account
            
            System.out.print("Please enter Interest Rate: ");
            double ir = keyboard.nextDouble();
            account = new Savings_Account(accountNumber, ir);
        }
        return account;
    }
